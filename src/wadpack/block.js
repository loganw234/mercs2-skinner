...
// entry table — `[u32w count][count × 16B (name_hash, type_hash, field_c, chunk_size)]`
...
import { pandemicHashM2, hex8 } from '../export.js';
import { u32w } from './bytes.js';
...
/** `[u32w count][count × 16B]` entry table at the head of a decompressed block. */
export function parseBlockEntryTable(decompressed) {
  if (decompressed.length < 4) return { count: 0, entries: [] };
  const count = u32w(decompressed, 0);
  const entries = [];
  for (let i = 0; i < count; i++) {
    const base = 4 + i * 16;
    if (base + 16 > decompressed.length) break;
    entries.push({
      nameHash: u32w(decompressed, base),
      typeHash: u32w(decompressed, base + 4),
      fieldC: u32w(decompressed, base + 8),
      chunkSize: u32w(decompressed, base + 12),
    });
  }
 ...
 *   [u32w 1][name_hash][type_hash][field_c=0][container_len][container...]
