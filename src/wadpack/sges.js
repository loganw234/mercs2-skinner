...
import { u16w, u32w, putU16, putU32w, align16 } from './bytes.js';
 ...
 *
 * The per-segment u16w compressed-size is unreliable for large/incompressible segments (it
...
export function decompressSges(block) {
  ...
  const segCount = u16w(block, 6);
  const totalU = u32w(block, 8);
  const tableStart = 16;
  ...
  for (let i = 0; i < segCount; i++) {
    ...
    const rawU = u16w(block, base + 2);
    const flagged = u32w(block, base + 4);
    segs.push({
      compSize: u16w(block, base),
...
export function compressSges(uncompressed, { segmentSize = DEFAULT_SEGMENT_SIZE, level = DEFAULT_LEVEL, major = 4 } = {}) {
  ...
  for (let off = 0; off < totalU; off += segmentSize) {
    ...
    // Fall back to raw storage when deflate overflows the u16w comp-size limit or fails to shrink.
  ...
  putU16(block, 6, n);
  putU32w(block, 8, totalU);
  putU32w(block, 12, totalC);

  for (let i = 0; i < n; i++) {
    const seg = segments[i];
    const base = 16 + i * 8;
    // comp_sz: full-size raw segments (>65535) overflow u16w, so store 0 (= default).
    ...
    putU32w(block, base + 4, (segOffsets[i] >>> 0) | (seg.isCompressed ? 1 : 0));
