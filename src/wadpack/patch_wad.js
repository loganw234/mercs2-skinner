...
import { u32w, putU32w, alignUp } from './bytes.js';
...
export function buildPatchWadMulti(blocks, csumValue = 0, csumMeta = null, certBlob = FFCS_CERT_BLOB) {
  ...
  const row = (at, tag, offset, m) => {
    ...
    putU32w(out, at + 4, offset); putU32w(out, at + 8, m);
  };
  ...
  layouts.forEach((L, i) => {
    ...
    putU32w(out, off, L.page);
    putU32w(out, off + 4, blocks[i].packedField);
    putU32w(out, off + 8, ((blocks[i].flags >>> 0) << 16) | L.pages);
  });
  ...
  allAset.forEach(([bi, e], i) => {
    ...
    putU32w(out, off, e.assetHash);
    putU32w(out, off + 4, e.u32_1);
    putU32w(out, off + 8, ((bi >>> 0) << 16) | (e.u32_2 & 0xffff));
    putU32w(out, off + 12, e.u32_3);
  });
...
export function readPatchWad(raw) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    chunks[tag] = [u32w(raw, off + 4), u32w(raw, off + 8)];
  }
  ...
  for (let i = 0; i < indxCount; i++) {
    ...
    indx.push([u32w(raw, off), u32w(raw, off + 4), u32w(raw, off + 8)]);
  }
  ...
  for (let i = 0; i < asetCount; i++) {
    ...
    const u0 = u32w(raw, off), u1 = u32w(raw, off + 4), u2 = u32w(raw, off + 8), u3 = u32w(raw, off + 12);
