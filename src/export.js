...
export function buildCommands({ bundle, plan }) {
  ...
  lines.push(
    ...
    ``,
    `# ⚠ KNOWN LIMITATION -- confirmed in game. This clones the RESIDENT block only, which`,
    `#   carries the coarsest LOD tier. The finer rungs live in a separate _P001 block that`,
    `#   holds pure geometry with no materials, and the engine finds it by PATH naming --`,
    `#   which --inject-extra cannot set. So a new-asset skin renders at its lowest detail`,
    `#   at ALL distances: on civ_hum_beachfemale_a that is 639 triangles instead of 3,856,`,
    `#   and the face visibly flattens.`,
    `#   For a pure recolour prefer the MODKIT export: it never clones the model, so the`,
    `#   original geometry and its full LOD chain are used.`,
    ``,
    `# 4. Wear it in-game`,
